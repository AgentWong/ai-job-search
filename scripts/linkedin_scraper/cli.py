"""
LinkedIn jobs-guest API scraper CLI.

Usage:
    .venv/bin/python -m scripts.linkedin_scraper.cli [OPTIONS]

Mirrors scripts/ats_scraper/cli.py: discovers postings, applies cheap regex
filters in Python, optionally stages survivors for LLM fuzzy review.
...
Options:
    ...
    --max-pages N                Max search pages per role (default: 3 -> ~30 cards)
    --max-per-role N             Max survivors to detail-fetch per role (default: 15)
    --search-delay S             Seconds between search requests (default: 3.0)
    --detail-delay S             Seconds between detail requests (default: 5.0)
    --experience-levels CSV      f_E filter values, e.g. '2,3,4' (default: unset)
    ...
    --output PATH                Summary JSON output (default: results/linkedin_api_results.json)
"""
...
from scripts.ats_scraper.config import JobPosting, load_config, load_exclusions
from scripts.ats_scraper.location import LocationConfig, posting_has_remote_signal
...
PENDING_REVIEW = RESULTS_DIR / "linkedin_pending_review.json"

TRACKER_NAME = "linkedin_api_role"
...
def _update_tracking(
    ...
    """
    Append per-role results to results/tracking/data/linkedin_api_role_effectiveness.csv
    and refresh its HTML dashboard.

    Uses the dedicated `linkedin_api_role` schema so long-term trend data for
    this workflow stays isolated from the shared `browser_role` tracker (which
    now records only hiringcafe; it historically also aggregated the retired
    browser linkedin + builtin workflows under each role).

    `qualified` count = pending records where regex_score >= 4 (the same
    score-threshold downstream consumers use for queue write). The downstream
    LLM review may further reject some, but that mirrors how the
    ats-api-search tracker writes its regex-qualified count too.
    """
    ...
    for rec in pending_records:
        role = rec.get("linkedin_role_searched", "")
...
def run(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        # Cap to max_per_role (priority order = newest first since sortBy=DD).
        # Runs after dedup so the cap reflects the deduped pool, not pre-dedup.
        cap_total_dropped = 0
        ...
        for role_name, cards in survivors_by_role.items():
            for card in cards:
                ...
                # Skip Easy Apply postings — these are LinkedIn's mass-applicant
                # funnel (no offsite apply button, just a one-click flow).
                # The LLM review would disqualify them anyway, so save the
                # review-tokens by dropping them before staging.
                if getattr(posting, "linkedin_easy_apply", False):
                    rejections["Easy Apply (LinkedIn)"] = (
                        rejections.get("Easy Apply (LinkedIn)", 0) + 1
                    )
                    if args.verbose:
                        print(f"    -> skipped: Easy Apply")
                    continue

                # Post-fetch remote gate (remote mode only). LinkedIn's guest-API
                # f_WT=2 (Remote) param is leaky — it returns fixed-metro on-site
                # cards anyway (observed 48/48 metro-located on a remote search).
                # The card-level filter can't catch a bare "Torrance, CA" card, so
                # we verify against the most authoritative text once fetched: drop
                # if neither the location/workplace_type nor the description shows
                # a remote arrangement. Mirrors the ATS gate's no_remote_signal.
                if loc_cfg.remote and not posting_has_remote_signal(
                    posting.location, posting.workplace_type, posting.description_text or ""
                ):
                    rejections["No remote signal (not remote)"] = (
                        rejections.get("No remote signal (not remote)", 0) + 1
                    )
                    if args.verbose:
                        print(f"    -> skipped: no remote signal (loc={posting.location!r})")
                    continue
                ...
                pending_records.append({
                    ...
                    "linkedin_role_searched": role_name,
                    "linkedin_seniority": getattr(posting, "linkedin_seniority", ""),
                    "linkedin_employment_type": getattr(posting, "linkedin_employment_type", ""),
                    "linkedin_easy_apply": getattr(posting, "linkedin_easy_apply", False),
                })
            else:
    ...
    if args.no_llm_review:
        ...
        print(f"Next step: orchestrator should invoke the linkedin-llm-review agent on this file")
    ...
    # Effectiveness tracking (linkedin_api_role schema, source=linkedin-api)
...
def main():
    parser = argparse.ArgumentParser(description="LinkedIn jobs-guest API scraper")
    ...
    parser.add_argument("--max-pages", type=int, default=3,
        help="Max search pages per role (default: 3 -> ~30 cards)")
    ...
    parser.add_argument("--detail-delay", type=float, default=5.0,
        help="Seconds between detail requests (default: 5.0)")
    parser.add_argument("--experience-levels", default=None,
        help="LinkedIn f_E filter as CSV, e.g. '2,3,4' for Entry/Associate/Mid-Senior")
    ...
    parser.add_argument("--output", default=str(RESULTS_DIR / "linkedin_api_results.json"),
