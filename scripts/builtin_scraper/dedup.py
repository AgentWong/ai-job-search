"""
Cross-role deduplication and spam removal for Builtin search results.

Builtin allows companies to post the same role to multiple cities, generating
...
1. **Spam pass** — count (normalized_company, normalized_title) occurrences
   ...
   3+ cities is gaming visibility. The signal is the duplication itself, so
   dropping one and keeping another doesn't help — discard the whole pair.
