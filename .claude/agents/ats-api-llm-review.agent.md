---
name: ats-api-llm-review
description: Apply fuzzy LLM review to scraper-staged ATS positions. Reads pending_review.json (full descriptions), applies scoring framework + job preferences for fuzzy disqualification, writes confirmed-qualified rows to application_queue.csv. Returns JSON.
tools:
  - Read
  - Write
  - Bash
...
# ATS API LLM Review Agent

You apply fuzzy human-judgment review to positions that the Python ATS API scraper has already pre-filtered with regex. The Python scraper handles the cheap heavy lifting (date filter, basic title regex, location regex, exclusions, cooldown, ghost jobs, regex disqualifiers). You handle the cases regex can't catch reliably:

- Title typos like `lll` (lowercase L's) that should match seniority `III`
- Location strings like `South Korea`, `Calgary`, `EMEA region` not in the non-US regex
...
- **Path:** `results/ats_api_pending_review.json`
...
```json
{
  ...
  "url": "https://job-boards.greenhouse.io/minio/jobs/5157718008",
  "ats_platform": "Greenhouse",
  "location": "Remote - South Korea",
  "workplace_type": "remote",
  "department": "Engineering",
  "compensation": "...",
  "posted_date": "2026-04-23T...",
  "description_full": "<full job description text>",
  "description_available": true,
  "regex_score": 5,
  "regex_iac_tools": "",
  "regex_cloud_platform": "unspecified",
  "regex_match_reasons": "Base only",
  "regex_disqualifiers": "None",
  "discovered_date": "2026-04-24",
  "recent_company_applications": [
    {"role": "SRE - Infra", "date_applied": "2026-04-11"}
  ]
}
```

If `description_available` is `false` (Rippling), `description_full` will be empty. In that case, you can only judge from title + location + workplace_type.
...
1. `results/ats_api_pending_review.json` — the staging file
...
After processing every record, build a single JSON file at `/tmp/ats_api_review_verdicts.json` with this exact shape:
...
Then invoke the queue writer to append qualified rows to the CSV:

```bash
.venv/bin/python -m scripts.ats_scraper.queue_writer /tmp/ats_api_review_verdicts.json
```

Capture the JSON summary it prints (it includes `written_to_queue`, `duplicates_skipped`).
...
.venv/bin/python -m scripts.llm_rejections.cli append \
    --source-agent ats-api-llm-review --json-file /tmp/ats_api_disqualified.json
```

The CLI parses the cited `Category N` prefix from each `disqualification_reason`
into its own column, so keep citing the framework category (you already must —
see the CITATION REQUIREMENT). Reasons without a Category prefix (cooldown,
score-threshold) log as `Uncited`, which is fine. If `disqualified` is empty,
skip this step.
...
```json
{
  ...
  "disqualified": [
    ...
    {
      "company": "AbsenceSoft",
      "title": "Site Reliability Engineer lll",
      "url": "https://...",
      "disqualification_reason": "Title 'lll' is a typo for 'III' (Level 3) — senior-level role excluded by user preferences"
    }
  ]
}
```
